...
namespace JQL
{
	public class DbDialog : IDisposable
	{
		...
		{
			DbColumn? dbColumn = Columns.FirstOrDefault(i => i.IsPrimaryKey == true) ?? throw new PowNetException("PrimaryKeyColumnIsNotDefined", System.Reflection.MethodBase.GetCurrentMethod())
		...
		public void Save()
        {
            if (_dbDialogsRoot is null) throw new PowNetException("DbDialogSaveWithNoPathIsNotPossible", System.Reflection.MethodBase.GetCurrentMethod())
                    .AddParam("DbDialog", ObjectName)
                    .GetEx();
		...
		public DbColumn GetTreeParentColumn()
		{
			...
			return dbColumn is null
				? throw new PowNetException("DbDialogIsNotTree", System.Reflection.MethodBase.GetCurrentMethod())
					.AddParam("DbDialog", ObjectName)
					.GetEx() : dbColumn;
		}
			...
			if (dbQuery is not null)
            {
                ...
                if (dbQuery.Columns is not null)
                {
					foreach (DbQueryColumn dbQueryColumn in dbQuery.Columns)
					{
						...
						if (!columnName.IsNullOrEmpty()) cqm.QueryColumns.TryAdd(columnName, false);
					}
				}
			}
			return cqm;
		...
		public static DbDialog Load(string dbDialogsRoot, string dbConfName, string? objectName)
        {
            ...
            if (!File.Exists(fp)) throw new PowNetException("FilePathIsNotExist", System.Reflection.MethodBase.GetCurrentMethod())
			...
			dbDialog = JsonSerializer.Deserialize<DbDialog>(dbDialogRaw) ?? throw new PowNetException("DeserializeError", System.Reflection.MethodBase.GetCurrentMethod())
					.AddParam("DbDialog", objectName.ToStringEmpty())
					.AddParam("FilePath", fp)
					.GetEx();
