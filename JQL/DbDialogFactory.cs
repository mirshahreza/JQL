...
namespace JQL
{
	public class DbDialogFactory
    {
        ...
        {
            DbDialogFolderPath = PowNetConfiguration.ServerPath;
            DbConfName = dbConfName;
            DbInfo = DatabaseConfiguration.FromSettings(DbConfName);
        ...
        public void DuplicateQuery(string objectName, string methodName, string methodCopyName)
        {
            ...
            DbQuery? dbQueryCopy = JsonExtensions.TryDeserializeTo<DbQuery>(tempString) ?? throw new PowNetException("DeserializeError", System.Reflection.MethodBase.GetCurrentMethod())
                    .AddParam("ObjectName", objectName)
        ...
        public void CreateServerObjectsFor(DbObject? dbObject, bool? createAdditionalUpdateByKeyQueries = true)
        {
            ...
            if (dbObject.DbObjectType == DbObjectType.Table)
            {
                ...
                cGen.DbDialogMethods.Add(nameof(QueryType.ReadList));
                cGen.DbDialogMethods.Add(nameof(QueryType.Create));
                cGen.DbDialogMethods.Add(nameof(QueryType.ReadByKey));
                cGen.DbDialogMethods.Add(nameof(QueryType.UpdateByKey));
                cGen.DbDialogMethods.Add(nameof(QueryType.Delete));
                cGen.DbDialogMethods.Add(nameof(QueryType.DeleteByKey));
            }
            ...
            //        DbColumn? dbColBy = dbDialog.Columns.FirstOrDefault(i => i.Name.EqualsIgnoreCase(dbColumn.Name + ExtString.UpdatedBy));
            //        DbColumn? dbColOn = dbDialog.Columns.FirstOrDefault(i => i.Name.EqualsIgnoreCase(dbColumn.Name + ExtString.UpdatedOn));
            //        if (dbColBy is not null || dbColOn is not null)
            //            CreateNewUpdateByKey(dbObject.Name, ExtString.ReadByKey, colsToUpdate, $"{dbColumn.Name}{ExtString.Update}", dbColBy is null ? "" : dbColBy.Name, dbColOn is null ? "" : dbColOn.Name, "");
            //    }
            //}
            DynamicCodeService.Refresh();
        }
